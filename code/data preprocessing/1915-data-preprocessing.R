...
####--------- API Request for San Diego and San Francisco 1915-1916 World's Fairs --------####

## As of Summer 2025, Chronicling America's dedicated API is deprecated.This script was updated in September of 2025 to reflect recent changes to the Library of Congress API and the JSON Response Objects to bulk data requests. Please refer to the LoC-API-Data-Dictionary.txt file.

## The base URL below is an advanced search for the phrase "international exposition" across all newspaper publications between 1915 and 1916.

## "&c=" facet in the URL is the number of matches per result page. Recent update allows up to 160 per page, but I recommend 100 as a parameter here. In any case make sure to keep track of how many results per page you're using.

## "&sb =" currently sorts query results by title. The option to sort by date (both ascending and descending) seems to be causing random duplicates in the API request. Final metadata files have been sorted by date (oldest to new), and the rowID reflects that.


####----------- Prep Steps ----------------####

# Define fixed base URL (first page of query results)
base_url <- "https://www.loc.gov/collections/chronicling-america/?c=100&dl=page&end_date=1916-12-31&ops=PHRASE&qs=international+exposition&searchType=advanced&start_date=1915-01-01&language=english&sb=title_s&fo=json"

# Define fair year that names data, metadata, and code files
fair_year <- "1915"

# Set a max_results equal to the matching results to the query
